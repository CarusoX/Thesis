...
\begin{frame}{Complejidad computacional}
  ...
  \begin{itemize}
    \item Permite evaluar y comparar la eficiencia de algoritmos
    \item Describe el comportamiento asintótico en función del tamaño de entrada
    \item Mide tiempo de ejecución y uso de memoria
  \end{itemize}
  
  \vspace{0.5cm}
  \textbf{En este trabajo:}
  \begin{itemize}
    \item Procesamos millones de puntos de datos
    \item La diferencia entre $O(n)$ y $O(n^2)$ puede ser minutos vs. días
    \item Optimización crucial para análisis en tiempo razonable
  \end{itemize}
  ...
  \note{
    ...
    En el procesamiento de señales, donde se manejan millones de puntos de datos, la diferencia entre algoritmos de complejidad O(n) y O(n²) puede resultar en diferencias de horas en el tiempo de procesamiento. Por ejemplo, para procesar 100 millones de puntos, considerando que un procesador realiza aproximadamente 10 a la 8 operaciones por segundo, un algoritmo O(n) podría completarse en segundos, mientras que un algoritmo O(n²) podría requerir días o semanas.
...
\begin{frame}{Notación Big-O}
  \textbf{Definición:}
  \begin{itemize}
    \item $f(n) = O(g(n))$ si existen constantes $c > 0$ y $n_0$ tales que
    \item $f(n) \leq c \cdot g(n)$ para todo $n \geq n_0$
  \end{itemize}
  
  \vspace{0.3cm}
  \centering
  \includegraphics[width=0.7\textwidth]{../escrito/figures/complejidades_algoritmicas.png}
...
\begin{frame}{Importancia de la constante en la complejidad}
  \textbf{La constante importa:}
  \begin{itemize}
    \item Por definición: $O(10n) = O(n)$ (la constante se elimina)
    \item Pero en la práctica: un algoritmo que hace 10 veces más operaciones requiere 10 veces más cómputo
  ...
  \note{
    Aunque por definición la notación nos dice que O(10n) es igual a O(n), porque la constante se elimina, es importante entender que un algoritmo que haga 10 veces más operaciones que otro, requerirá obviamente 10 veces más cómputo.
    
    Por esta razón, es importante no solo prestar atención a la complejidad de los algoritmos, sino también a la constante que afecta a esta, ya que este factor puede ser determinante en la práctica.
    
    En este trabajo tenemos un ejemplo claro: el algoritmo original de detección de pulsos tiene una complejidad O(100000 por n), mientras que el algoritmo optimizado tiene O(200 por n). Ambos son O(n), pero el optimizado es 500 veces más rápido debido a la reducción de la constante.
    
    Por lo tanto, optimizar la constante es crucial para hacer viable el análisis de grandes volúmenes de datos en tiempo razonable.
  }
\end{frame}
...
\begin{frame}{Estructuras de datos básicas}
  \begin{columns}[t]
    ...
    \begin{minipage}[t]{\textwidth}
      \textbf{Array (Arreglo):}
      \begin{itemize}
        \item Lista de elementos en un espacio contiguo de memoria
        \item Acceso directo a cualquier elemento en $O(1)$
      \end{itemize}
    \end{minipage}
    ...
    \begin{minipage}[t]{\textwidth}
      ...
      \begin{itemize}
        ...
        \item Operaciones: \texttt{push}, \texttt{pop} en $O(1)$
        \item Lectura: \texttt{front} en $O(1)$
      \end{itemize}
    \end{minipage}
  \end{columns}
  ...
  \begin{columns}[t]
    ...
    \vspace{0pt}
    \begin{minipage}[t]{\textwidth}
      \textbf{Deque (Cola doble):}
      \begin{itemize}
        \item Inserción/eliminación por ambos extremos
        \item Operaciones: \texttt{push\_back}, \texttt{push\_front}, \texttt{pop\_back}, \texttt{pop\_front} en $O(1)$
        \item Lectura: \texttt{front}, \texttt{back} en $O(1)$
      \end{itemize}
    \end{minipage}
    ...
    \begin{minipage}[t]{\textwidth}
      \textbf{Ejemplos:}
      \begin{itemize}
        \item \textbf{Array:} lista de supermercado
        \item \textbf{Queue:} cola del supermercado
        \item \textbf{Deque:} escalera mecánica bidireccional
      \end{itemize}
    \end{minipage}
  \end{columns}
  
  \note{
    Para entender las estructuras de datos básicas, comenzamos con el array, que es un espacio contiguo de memoria que permite acceso directo a cualquier índice en tiempo constante O(1).
    
    Una cola, o queue, es una estructura de datos que sigue el principio FIFO - First In, First Out - donde los elementos se agregan por un extremo y se eliminan por el otro. Un ejemplo clásico es la cola del supermercado. Una queue tiene dos operaciones básicas: push, que agrega un elemento al principio de la queue, y pop, que elimina el elemento al final de la queue.
    
    Una cola doblemente terminada, o deque, es una estructura de datos que permite insertar y eliminar elementos por ambos extremos. Un ejemplo clásico es una escalera mecánica que pueda cambiar de dirección. La deque tiene operaciones de inserción y eliminación: push_back, que agrega un elemento al final, y pop_front, que elimina el elemento al principio. También tiene operaciones de lectura: front, para leer el elemento del frente, y back, para leer el elemento del final.
    
    Todas estas operaciones se pueden realizar en tiempo constante, O(1).
  }
\end{frame}
...
\begin{frame}{Lectura de datos}
  \begin{itemize}
    \item Formato de entrada: archivo `.lvm' con 3 columnas
    \begin{itemize}
      \item Tiempo
      \item Señal del anillo
      \item Señal de la placa
    \end{itemize}
  \end{itemize}
...
\begin{frame}{Preprocesamiento de señales}
  \begin{enumerate}
    \item \textbf{Rellenado de huecos:} Interpolación lineal entre puntos anterior y posterior
    \item \textbf{Remoción de offset:} Para cada punto, se calcula el promedio de 5000 puntos circundantes y se resta
  \end{enumerate}
  
  \note{
    El programa de preprocesamiento realiza dos operaciones fundamentales.
    
    Primero, el rellenado de huecos mediante interpolación. Debido a las limitaciones de hardware, cada segundo se pierden aproximadamente 50 milisegundos de datos durante la adquisición. El programa identifica estos huecos y los rellena utilizando interpolación lineal entre el punto anterior y el posterior al hueco.
    
    Segundo, la remoción del offset de las señales. Para cada punto de la señal, se calcula el promedio de los 5000 puntos circundantes - 2500 puntos a cada lado - y se le resta este valor. Para esto se ignoran los primeros y últimos 2500 puntos de la señal. Este proceso elimina el offset de corriente continua que puede variar durante la medición.
    
    Finalmente, se escriben las señales preprocesadas en un archivo. El formato de salida son 4 columnas: contador de tiempo como entero, señal del anillo resultante, señal de la placa resultante y un booleano que indica si la fila corresponde a un hueco.
  }
\end{frame}

\begin{frame}{Detección de pulsos}
  \textbf{Sistema de umbrales adaptativos:}
  \begin{itemize}
    \item 1000 umbrales desde 2.00 hasta 0.02
    \item Para cada umbral, busca pares de puntos consecutivos que superen el umbral en ambas señales
    \item Separación máxima de 100 puntos entre señales
  \end{itemize}
  
  \vspace{0.3cm}
  \centering
  \includegraphics[width=0.7\textwidth]{../escrito/figures/sensores_gota.png}
...
\begin{frame}{Cálculo de propiedades físicas}
  \begin{columns}
    ...
    \textbf{Velocidad:}
    \begin{itemize}
      \item Separación conocida: 5.7 cm
      \item Dividida por tiempo entre p1 y p2
    \end{itemize}
...
\begin{frame}{Filtros de calidad}
  ...
  Una vez detectados y delimitados los pulsos, se aplican filtros de calidad para separar los pulsos válidos de los falsos positivos.

  \vspace{0.5cm}

  \textbf{Filtros:}
  \begin{enumerate}
    \item \textbf{Carga:} $|q| > 0.2$ en ambos canales
...
\begin{frame}{Sistema de evaluación y ordenamiento}
  ...
  Una vez tenemos los pulsos, los evaluamos para determinar que tan buenos son. Las metricas fueron desarrolladas mediante ajuste manual y experimental.

  \vspace{0.2cm}
  
  \textbf{5 criterios de evaluación:}
  \begin{itemize}
    \item \textbf{Eval1, Eval2:} Diferencia cuadrática con modelos teóricos del sensor del anillo y la placa respectivamente
    \item \textbf{Eval3:} Desviación de proporción carga (87\% anillo/placa)
    \item \textbf{Eval4:} Desviación de proporción ancho (79\% placa/anillo)
    \item \textbf{Eval5:} Proporción de señal en rango de ruido
  \end{itemize}
  
  \vspace{0.2cm}
  \textbf{Puntuación total:}
  $$eval = \ln(1 + eval1) + \ln(1 + eval2) + 3 \cdot eval3 + 3 \cdot eval4 + eval5^2$$
  
  \vspace{0.2cm}
  Menor puntuación = mejor calidad (rango típico: $[0, 20]$)
  ...
  \note{
    ...
    Eval1 y Eval2 miden la diferencia cuadrática entre la integral de la señal real y dos modelos teóricos distintos del pulso, uno para el anillo y otro para la placa.
    
    Eval3 mide la desviación de la proporción esperada entre cargas del anillo y la placa. Experimentalmente se observó que la carga medida en el anillo es aproximadamente el 87 por ciento de la carga medida en la placa.
    
    Eval4 mide la desviación de la proporción esperada del ancho entre los picos de la señal de la placa y el anillo. Experimentalmente se observó que el ancho del pulso de la placa es aproximadamente el 79 por ciento del ancho del pulso del anillo.
    
    Eval5 mide la proporción de señal en el rango de ruido, entre -0.02 y 0.02.
    
    Las evaluaciones se combinan para obtener una puntuación total. Mientras más chico sea el valor de la puntuación total, mejor es la calidad de la gota. El rango típico está entre 0 y 20.
  }
\end{frame}
...
\begin{frame}{Problemas de diseño}
  \begin{columns}[t]
    ...
    \vspace{0pt}
    \begin{itemize}
      \item \textbf{Sin modularización:} Todo en entorno global
      \item \textbf{Variables sin contexto:} `s', `ss', `i', `j', `w', `k', etc.
      \item \textbf{Código duplicado:} Lógica repetida con pequeños cambios
    \end{itemize}
    
    \column{0.5\textwidth}
    \vspace{0pt}
    \begin{itemize}
      \item \textbf{I/O hard-codeado:} Nombres de archivos en código
      \item \textbf{División forzada:} Alto consumo de memoria obliga a procesar por partes
      \item \textbf{Pérdida de datos:} Pulsos en medio de cortes no se procesan
    \end{itemize}
  \end{columns}
  
  \vspace{0.5cm}
  Cualquier modificación requiere entender todo el código
...
\begin{frame}{Problemas de rendimiento}
  \textbf{Remoción de offset:}
  \begin{itemize}
    \item Para cada punto: suma de 5001 valores y 5001 divisiones
    \item Complejidad: $O(5001n) = O(n)$ pero con constante muy alta
  \end{itemize}
  
  \vspace{0.5cm}
  \textbf{Detección de pulsos:}
  \begin{itemize}
    \item 1000 umbrales $\times$ todos los puntos $\times$ 100 puntos de búsqueda
    \item Complejidad: $O(100000 \cdot n) = O(n)$ pero con constante extremadamente alta
  \end{itemize}
  
  \vspace{0.3cm}
  \centering
  \textcolor{red}{\textbf{Ambos son $O(n)$ pero con constantes muy altas}}
  
  \note{
    Al analizar el rendimiento del código, se encontraron dos áreas de potenciales mejoras.
    
    Primero, el algoritmo de remoción de offset es muy ineficiente. Para cada punto, se hace una suma de 5001 valores divididos por 5001, resultando en una complejidad O(5001n) que es O(n). Obviamente se trata de una complejidad lineal, pero no solo que la constante es altísima por el factor de 5001, sino que también se realizan 5001 divisiones por cada punto, que no es la operación más eficiente en un procesador.
    
    Segundo, la detección de pulsos. Si analizamos la complejidad de la implementación de este algoritmo, podemos ver que para cada umbral - 1000 umbrales - se revisa todo el array de las señales. Si dos puntos consecutivos superan el umbral en la primera señal, se revisa hasta un total de 100 puntos en la segunda señal. Esto resulta en una complejidad O(100000 por n) que es O(n). Idéntico al caso anterior, se trata de una complejidad lineal, pero con una constante extremadamente alta.
    
    Ambos algoritmos son O(n) pero con constantes muy altas, lo que los hace extremadamente lentos en la práctica.
  }
\end{frame}

\begin{frame}{Motivación de la refactorización}
  \textbf{Beneficios de la refactorización:}
  \begin{itemize}
    \item \textbf{Mantenibilidad:} Menos código por revisar, lógica en funciones pequeñas
    \item \textbf{Reutilizabilidad:} Código reutilizable para otros experimentos o instrumentos
    \item \textbf{Verificación:} Pruebas unitarias de cada función, no del programa completo
    \item \textbf{Colaboración:} Otros investigadores pueden entender y modificar el código más fácilmente
  \end{itemize}
  
  \vspace{0.5cm}
  \centering
  \textbf{Necesidad:} Resolver problemas de diseño y rendimiento para mejorar el sistema
...
\begin{frame}{Ejemplo MinQueue - Parte 1}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_1.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_2.png}
  \end{columns}
...
\begin{frame}{Ejemplo MinQueue - Parte 2}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_3.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_4.png}
  \end{columns}
...
\begin{frame}{Ejemplo MinQueue - Parte 3}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_5.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_6.png}
  \end{columns}
...
\begin{frame}{Ejemplo MinQueue - Parte 4}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_7.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_8.png}
  \end{columns}
...
\begin{frame}{Ejemplo MinQueue - Parte 5}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_9.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_10.png}
  \end{columns}
...
\begin{frame}{Ejemplo MinQueue - Parte 6}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_11.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_12.png}
  \end{columns}
...
\begin{frame}{Ejemplo MinQueue - Parte 7}
  \begin{columns}
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_13.png}
    
    \column{0.5\textwidth}
    \centering
    \includegraphics[width=0.95\textwidth]{figures/minqueue_14.png}
  \end{columns}
