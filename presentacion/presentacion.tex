...
\begin{frame}{Problemas y objetivos}
  ...
  \begin{itemize}
    \item \textbf{Es muy lento:} 6 horas para procesar una tormenta de 5 horas (100M datos)
    \item \textbf{Consume mucha memoria:} obliga a procesar los datos de a partes
    \item \textbf{Código monolítico:} Requiere entender todo el código para hacer modificaciones
  \end{itemize}
  ...
  \note{
    ...
    También es necesario refactorizar el código para hacerlo más modular y mantenible, lo cual facilita la implementación de nuevas funcionalidades y la corrección de errores.
  }
\end{frame}
...
\begin{frame}{Importancia de la constante en la complejidad}
  ...
  En general, la función $g(n)$ es lo más simple posible, es decir que si
  \begin{itemize}
    \item $f(n) = O(n)$
    \item $f(n) = O(n^2)$
    \item $f(n) = O(3n + 7)$
  ...
  \begin{itemize}
    ...
    \item Por lo tanto, es importante prestar atención a la magnitud de la constante de un algoritmo, para terminar de evaluar su desempeño en la práctica.
  \end{itemize}
\end{frame}
...
\begin{frame}{Remoción de offset}
    ...
    \end{center}
    \vspace{0.2cm}
    La implementación actual tiene complejidad $O(10002n) = O(n)$
\end{frame}

\begin{frame}{Detección de pulsos}
  \textbf{Búsqueda por umbrales:}
  \begin{itemize}
...
\begin{frame}{Ejemplo MinQueue - Parte 1}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_1.png}
  \vspace{0.5cm}
  \includegraphics[width=0.8\textwidth]{figures/min_queue_2.png}
...
\begin{frame}{Ejemplo MinQueue - Parte 2}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_3.png}
  \vspace{0.5cm}
  \includegraphics[width=0.8\textwidth]{figures/min_queue_4.png}
...
\begin{frame}{Ejemplo MinQueue - Parte 3}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_5.png}
\end{frame}

\begin{frame}{Ejemplo MinQueue - Parte 4}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_6.png}
  \vspace{0.5cm}
  \includegraphics[width=0.8\textwidth]{figures/min_queue_7.png}
...
\begin{frame}{Ejemplo MinQueue - Parte 5}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_8.png}
  \vspace{0.5cm}
  \includegraphics[width=0.8\textwidth]{figures/min_queue_9.png}
...
\begin{frame}{Ejemplo MinQueue - Parte 6}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_10.png}
  \vspace{0.5cm}
  \includegraphics[width=0.8\textwidth]{figures/min_queue_11.png}
...
\begin{frame}{Ejemplo MinQueue - Parte 7}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_12.png}
...
\begin{frame}{Ejemplo MinQueue - Parte 8}
  \centering
  \includegraphics[width=0.8\textwidth]{figures/min_queue_13.png}
  \vspace{0.5cm}
  \includegraphics[width=0.8\textwidth]{figures/min_queue_14.png}
...
\begin{frame}{Nuevo flujo de trabajo}
  \begin{center}
    \includegraphics[width=0.8\textwidth]{figures/diagrama_nuevo.png}
  \end{center}
\end{frame}
...
\begin{frame}{Optimización en el algoritmo de remoción de offset}
  ...
  \begin{itemize}
    ...
    \item Complejidad: $O(10002n) = O(n)$ con constante muy alta
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item Reducción de \textbf{10002 operaciones} a solo \textbf{3 operaciones} por punto
    \item Misma complejidad: $O(n)$, pero con constante mucho menor
  \end{itemize}
  
  \vspace{0.3cm}
  \centering
  \textcolor{red}{\textbf{Reducción de $\sim 3300$ veces en operaciones por punto}}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 1}
  Aprovechar que los pulsos miden máximo 400 puntos
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_0.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 2}
  Un pulso puede quedar en el medio si buscamos en solo los primeros 400 puntos
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_1.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 3}
  Buscamos el punto crítico del sensor 1 entre los primeros 400 puntos, pero permitimos que el punto crítico del sensor 2 esté en la siguiente ventana
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_2.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 4}
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_3.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 5}
  Búsqueda con MinMaxQueue de tamaño 100 sobre el sensor 2 para obtener el mejor crítico para cada punto del sensor 1
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_4.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 6}
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_5.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 7}
  Delimitamos el pulso encontrado, y podemos calcular sus propiedades y aplicar los filtros de calidad
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_6.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 8}
  En caso de que sea válido, marcamos como utilizado todos los puntos del pulso y reiniciamos la búsqueda en la misma ventana
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_7.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 9}
  Se permiten 100 puntos utilizados al inicio de la ventana, para encontrar mejor el inicio del pulso
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_8.png}
  \end{center}
\end{frame}

\begin{frame}{Cambios en la detección de pulsos - Parte 10}
  Si se encuentra un pulso inválido, se marcan como utilizados los puntos críticos encontrados y se reinicia la búsqueda en la misma ventana
  \begin{center}
    \includegraphics[width=0.9\textwidth]{figures/mejora_busqueda_9.png}
  \end{center}
\end{frame}

\begin{frame}{Análisis de complejidad}
  \begin{itemize}
    \item El algoritmo procesa $O(n/300)$ ventanas, donde $n$ es el número total de puntos
    \item Cada ventana tiene tamaño $V = 800$ puntos, pero solo nos interesan 400 de ellos
    \item Se marcan como mínimo 4 puntos por búsqueda $\Rightarrow$ cota superior de $400/4 = 100$ búsquedas por ventana
    \item Cada búsqueda tiene complejidad $O(400)$
  \end{itemize}
  
  \vspace{0.5cm}
  
  \textbf{Complejidad total:}
  $$O\left(\frac{n}{300} \times 100 \times 400 \right) = O(133n) = O(n)$$
  
  \vspace{0.3cm}
  
  \textcolor{red}{\textbf{Reducción de constante: de 100000 a 133 (750 veces)}}
\end{frame}
...
\vspace{0.5cm}
\begin{center}
  \textcolor{red}{\textbf{7200\% de mejora en tiempo de procesamiento}}
\end{center}
\vspace{0.3cm}
\end{frame}
...
\begin{frame}{Posibles mejoras}
  \begin{itemize}
    ...
    \item Paralelizar el procesamiento de datos para reducir aún más los tiempos de ejecución.
