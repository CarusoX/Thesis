...
Una cola (queue) es una estructura de datos que sigue el principio FIFO (First In, First Out), donde los elementos se agregan por un extremo y se eliminan por el otro. La \textit{MinQueue} extiende este concepto manteniendo una cola auxiliar ordenada para acceder al elemento mínimo en tiempo constante. En lugar de buscar el mínimo en toda la ventana cada vez (operación O(n)), mantiene una estructura auxiliar que permite obtener el mínimo en O(1).
