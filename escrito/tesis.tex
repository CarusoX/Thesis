...
% Variables para la carátula
\newcommand{\nombreautor}{Uziel Ludueña}
\newcommand{\fechadefensa}{Diciembre, 2025}
...
\begin{titlepage}
    ...
    \begin{minipage}{0.45\textwidth}
        \raggedright
        \includegraphics[height=1.5cm,keepaspectratio]{figures/unc_logo.jpg}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\textwidth}
        \raggedleft
        \includegraphics[height=1.5cm,keepaspectratio]{figures/famaf_logo_cropped.png}
    \end{minipage}
    
    \vspace{1.5cm}
    
    % Título del trabajo
    {\Large Desarrollo de un sistema eficiente para el procesamiento de datos experimentales de la carga eléctrica de gotas de lluvia}
    ...
    % Información de la facultad y grado
    \begin{minipage}{0.9\textwidth}
        \centering
        Presentado ante la FACULTAD DE MATEMÁTICA, ASTRONOMÍA, FÍSICA Y COMPUTACIÓN como parte de los requerimientos para la obtención del grado de Licenciado/a en Ciencias de la Computación de la
    \end{minipage}
    ...
    \vfill
    \includegraphics[height=1.2cm,keepaspectratio]{figures/copyright.png}
    
    \vspace{0.1cm}
    \begin{minipage}{0.85\textwidth}
        \centering
        \small
        Este trabajo se distribuye bajo una licencia \href{https://creativecommons.org/licenses/by/4.0/}{Creative Commons Atribución-NoComercial 4.0 Internacional}.
    \end{minipage}
...
\begin{itemize}
    \item \textbf{$O(1)$ - Constante:} El tiempo de ejecución es independiente del tamaño de la entrada, como cuando se accede a una posición fija de un arreglo o a una celda de una tabla hash con direccionamiento directo. \cite{cormen2009introduction}
    
    \item \textbf{$O(\log n)$ - Logarítmica:} El tiempo crece logarítmicamente al dividir repetidamente el espacio de búsqueda, tal como ocurre en la búsqueda binaria sobre arreglos ordenados o árboles balanceados. \cite{kleinberg2006algorithm}
    
    \item \textbf{$O(n)$ - Lineal:} El tiempo es proporcional al tamaño de la entrada; recorrer secuencialmente todos los elementos de un arreglo o calcular frecuencias de aparición son patrones típicos. \cite{sedgewick2011algorithms}
    
    \item \textbf{$O(n \log n)$ - Lineal logarítmica:} El tiempo crece como $n$ multiplicado por $\log n$, característico de `algoritmos de divide y vencerás' como merge sort o heapsort. \cite{skiena2012algorithm}
    
    \item \textbf{$O(n^2)$ - Cuadrática:} El tiempo es proporcional al cuadrado de la entrada, como en bubble sort o en algoritmos que comparan cada par de elementos. \cite{aho1974design}
\end{itemize}
...
\begin{itemize}
    ...
    Finalmente, se calculan las evaluaciones de la siguiente manera:

    \begin{equation}
        eval1 = \sum_{i=0}^{puntos\_totales} (a1(i) - integral\_sensor1(i))^{2} 
    \end{equation}

    \begin{equation}
        eval2 = \sum_{i=0}^{puntos\_totales} (a2(i) - integral\_sensor2(i))^{2} 
    \end{equation}
    ...
    \begin{equation}
        eval3 = \left|\frac{q_1}{q} - 0.87 \cdot \frac{q_2}{q}\right|
    \end{equation}
...
señal de la placa y el anillo. Experimentalmente se observó que el ancho del pulso de la placa es aproximadamente el 79\% del ancho del pulso del anillo, por lo que se mide la desviación de esta proporción.
    ...
    \begin{equation}
        eval4 = \left|\frac{p_1}{p} - 0.79 * \frac{p_2}{p}\right|
    \end{equation}
    ...
    \begin{equation}
        eval5 = \frac{\#\{p: |sensor1(p)| <= 0.02\} + \#\{p: |sensor2(p)| <= 0.02\}}{puntos\_totales}
    \end{equation}

\end{itemize}

    Las evaluaciones se combinan para finalmente obtener una puntuación total, que está generalmente en el rango $[0, 20]$ de la siguiente manera:

    \begin{equation}
        eval = ln(1 + eval1) + ln(1 + eval2) + 3 * eval3 + 3 * eval4 + eval5^{2}
    \end{equation}

    Mientras más chico sea el valor de la puntuación total, mejor es la calidad de la gota.
    
    % En la figura \ref{fig:puntuacion_calidad} se puede observar la puntuación de calidad para diferentes gotas.
...
\begin{itemize}
    ...
    resultando en una complejidad $$O(10002n) = O(n)$$ Obviamente se trata de una complejidad lineal, pero no solo que la constante es altísima por el factor de 10002, sino que también se realizan 5001 divisiones por cada punto, que no es la operación más eficiente en un procesador \cite{Lemire2019FasterRemainder}.
